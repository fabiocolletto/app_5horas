...
function createActionButton(label, background, color = '#0b132b') {
...
export const cell = {
  ...
  init(context) {
    ...
    const homeButton = createActionButton('Voltar para home', '#3a506b', '#e0e6ed');
    homeButton.addEventListener('click', () => context.navigate('home'));

    const educationButton = createActionButton('Ir para Educação', '#5bc0be');
    educationButton.addEventListener('click', () => context.navigate('education'));
  ...
  destroy() {
    // Não há listeners globais ou timers para limpar nesta célula.
  },
};
